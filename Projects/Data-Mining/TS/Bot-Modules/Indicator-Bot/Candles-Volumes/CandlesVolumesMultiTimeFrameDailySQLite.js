exports.newDataMiningBotModulesCandlesVolumesMultiTimeFrameDailySQLite = function (processIndex) {

    const MODULE_NAME = "Candles Volumes Multi Time Frame Daily SQLite"
    ...
    function start(callBackFunction) {
        try {
            ...
            function buildCandles() {
                try {
                    ...
                    function processTimeFrames() {
                        ...
                        let timeFrames = TS.projects.foundations.globals.timeFrames.dailyTimeFramesArray();
                ...
                try {
                    ...
                    function writeCandles() {
                        ...
                        function onFileCreated(err) {
                            ...
                            TS.projects.foundations.globals.loggerVariables.VARIABLES_BY_PROCESS_INDEX_MAP.get(processIndex).BOT_MAIN_LOOP_LOGGER_MODULE_OBJECT.write(MODULE_NAME,
                                "[INFO] writeCandles -> Created " + fileRecordCounter + " candle records for " + timeFrame + " on " + dateForPath);
                            writeVolumes()
                        }
                    }
                    ...
                    function writeVolumes() {
                        ...
                        function onFileCreated(err) {
                            ...
                            TS.projects.foundations.globals.loggerVariables.VARIABLES_BY_PROCESS_INDEX_MAP.get(processIndex).BOT_MAIN_LOOP_LOGGER_MODULE_OBJECT.write(MODULE_NAME,
                                "[INFO] writeVolumes -> Created " + fileRecordCounter + " volume records for " + timeFrame + " on " + dateForPath);
            ...
            function writeDataRanges(callBack) {
                try {
                    writeDataRange(contextVariables.datetimeBeginingOfMarketFile, contextVariables.datetimeLastProducedFile, CANDLES_FOLDER_NAME, (err) => {
                        if (err.result !== TS.projects.foundations.globals.standardResponses.DEFAULT_OK_RESPONSE.result) {
                            callBack(err)
                            return
                        }
                        writeDataRange(contextVariables.datetimeBeginingOfMarketFile, contextVariables.datetimeLastProducedFile, VOLUMES_FOLDER_NAME, callBack);
                    });
                }
                catch (err) {
                    TS.projects.foundations.globals.processVariables.VARIABLES_BY_PROCESS_INDEX_MAP.get(processIndex).UNEXPECTED_ERROR = err
                    TS.projects.foundations.globals.loggerVariables.VARIABLES_BY_PROCESS_INDEX_MAP.get(processIndex).BOT_MAIN_LOOP_LOGGER_MODULE_OBJECT.write(MODULE_NAME,
                        "[ERROR] writeDataRanges -> err = " + err.stack);
            ...
            function writeDataRange(pBegin, pEnd, pProductFolder, callBack) {
                try {
                    ...
                    function onFileCreated(err) {
                        if (err.result !== TS.projects.foundations.globals.standardResponses.DEFAULT_OK_RESPONSE.result) {
                            TS.projects.foundations.globals.loggerVariables.VARIABLES_BY_PROCESS_INDEX_MAP.get(processIndex).BOT_MAIN_LOOP_LOGGER_MODULE_OBJECT.write(MODULE_NAME,
                                "[ERROR] writeDataRange -> onFileCreated -> err = " + err.stack)
                            callBack(err)
                            return
                        }
                    ...
                    TS.projects.foundations.globals.processVariables.VARIABLES_BY_PROCESS_INDEX_MAP.get(processIndex).UNEXPECTED_ERROR = err
                    TS.projects.foundations.globals.loggerVariables.VARIABLES_BY_PROCESS_INDEX_MAP.get(processIndex).BOT_MAIN_LOOP_LOGGER_MODULE_OBJECT.write(MODULE_NAME,
                        "[ERROR] writeDataRange -> err = " + err.stack)
                    callBack(TS.projects.foundations.globals.standardResponses.DEFAULT_FAIL_RESPONSE)
                }
            }
